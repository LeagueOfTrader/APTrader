...
{	
	...
	if (m_trade != NULL) {
		m_trade->onTraded(instrumentID, TT_Open, price, volume, orderID, direction);
	}
	return sysID;
}
...
APSYSTEMID APSimTradeSystem::requestClose(APORDERID orderID, APASSETID instrumentID, APTradeDirection direction, double price, long volume)
{
	...
	m_orderList.push_back(order);
	if (m_trade != NULL) {
		m_trade->onTraded(instrumentID, TT_Close, price, volume, orderID, direction);
	}
	return sysID;
}
...
{
	if (m_trade != NULL) {
		m_trade->onTraded(instrumentID, type, price, volume, orderID, direction);
